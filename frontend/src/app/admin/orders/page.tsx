...
import { Package, MapPin, CheckCircle, Clock, Navigation, AlertCircle, Eye, RefreshCw, Search, ListFilter, Truck, User } from "lucide-react";
...
const statusMap: Record<string, { label: string; color: string; badge: string }> = {
  REQUESTED: { label: "طلب جديد (مراجعة)", color: "text-amber-400", badge: "bg-amber-400/20 text-amber-400 border border-amber-400/30" },
  ...
  PICKED_UP: { label: "تم الاستلام من المرسل", color: "text-purple-400", badge: "bg-purple-400/20 text-purple-400 border border-purple-400/30" },
  ...
  CANCELED: { label: "ملغي", color: "text-red-500", badge: "bg-red-500/20 text-red-500 border border-red-500/30 font-bold" },
};
...
export default function AdminOrdersPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || "تعذر تحميل قائمة الطلبات في المنصة");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (filter !== "ALL") {
      if (filter === "ACTIVE") {
        ...
        result = result.filter((o) => ["REQUESTED", "PENDING"].includes(o.status));
      }
    }
    ...
    if (search.trim() !== "") {
      ...
      result = result.filter(
        (o) =>
          ...
          (o.customer && `${o.customer.firstName} ${o.customer.lastName}`.toLowerCase().includes(query)) ||
          (o.driver?.user && `${o.driver.user.firstName} ${o.driver.user.lastName}`.toLowerCase().includes(query))
      );
    }
    ...
    <div className="space-y-8 pb-16 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-4">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2">
            ...
            مراقبة وتتبع جميع طلبات المنصة ({filteredOrders.length} طلب)
          </h1>
          <p className="text-xs text-muted mt-1">
            متابعة حية لحالة الشحنات، بيانات العملاء والكباتن، والتتبع الجغرافي الفوري
          </p>
        </div>
        <button
          onClick={fetchOrders}
          className="p-2.5 rounded-xl border border-white/10 bg-surface text-muted hover:text-white hover:border-white/20 transition-all flex items-center gap-1.5 text-xs font-bold shadow-md"
        >
      ...
      <div className="bg-surface border border-white/10 rounded-3xl p-6 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2 w-full sm:w-80 bg-background border border-white/10 rounded-2xl px-4 py-2.5">
          ...
          <input
            ...
            placeholder="بحث برقم الطلب، العميل، المندوب، أو العنوان..."
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0">
          {[
            ...
            { key: "ACTIVE", label: "الطلبات النشطة (قيد التوصيل)" },
            { key: "PENDING", label: "طلبات بانتظار مندوب" },
            { key: "DELIVERED", label: "مكتملة ومسلمة" },
          ].map((btn) => (
            <button
              key={btn.key}
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-surface border border-white/10 rounded-3xl p-8 shadow-xl text-muted">
          <Package className="w-16 h-16 mx-auto mb-3 opacity-30 text-gold" />
          <h3 className="text-lg font-bold text-white mb-1">لا توجد طلبات مطابقة للبحث أو التصفية</h3>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex items-start gap-4 flex-1 relative z-10">
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-xs pt-2 border-t border-white/5 font-semibold text-muted">
                      <div className="flex items-center gap-1">
                        <User className="w-3.5 h-3.5 text-amber-400" />
                        <span>المرسل: <strong className="text-white">{custName}</strong> {order.customer?.companyName ? `(${order.customer.companyName})` : ""}</span>
                      </div>
                      ...
                      {order.driver?.user ? (
                        ...
                        <div className="bg-amber-400/10 text-amber-400 border border-amber-400/20 px-2.5 py-1 rounded-lg text-[10px] font-bold">
                          ⏳ بانتظار قبول مندوب
                        </div>
                      )}
                      {order.receiverName && <div>المستلم: <strong className="text-white">{order.receiverName}</strong></div>}
